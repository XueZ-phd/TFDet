...
class FasterRCNN_RGBTwMask_wSpaAttV2_Lovasz_Sigmoid(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 dice_weight,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV2_Lovasz_Sigmoid, self).__init__(init_cfg=init_cfg)
...
class TargetAwareFusion(nn.Module):
    ...
    def forward(self, rgb_x, lwir_x, gt_masks):
        ...
        for layer_idx, (tmp_rx, tmp_lx, tmp_fusion_name, tmp_mask_name, tmp_spaAtt_name) in \
            ...
            refined_tmp_fused_res = similarity_value/torch.max(similarity_value, 1, keepdim=True)[0] * tmp_fused_res
