...
class FasterRCNN_RGBTwMask_wSpaAttV2_LLVIP(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 taf_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV2_LLVIP, self).__init__(init_cfg=init_cfg)
    ...
    def extract_feat(self, rgb_img, lwir_img, gt_masks=None):
        ...
        # save_dir = '/home/zx/cross-modality-det/code/mmdetection/runs_llvip/FasterRCNN_r50wMask_ROIFocalLoss5_CIOU20_cosineSE_notDetach_negEntropy1/rgbt_feature_cosine_similarities'
        # save_dir_s0 = os.path.join(save_dir, 'scale0')
        # save_dir_s1 = os.path.join(save_dir, 'scale1')
        # save_dir_s2 = os.path.join(save_dir, 'scale2')
        ...
        # # scale 2
        # x0, x1 = rgb_x[2], lwir_x[2]
        # cosine = F.cosine_similarity(x0.flatten(2).unsqueeze(1), x1.flatten(2).unsqueeze(2),
        #                              dim=3).squeeze().detach().cpu().numpy()
        # n_npf = len(glob(os.path.join(save_dir_s2, '*.npy')))
        # np.save(os.path.join(save_dir_s2, f'{n_npf:04d}.npy'), cosine)
