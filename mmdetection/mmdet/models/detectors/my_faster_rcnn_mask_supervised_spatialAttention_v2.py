...
class FasterRCNN_RGBTwMask_wSpaAttV2(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 dice_weight,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV2, self).__init__(init_cfg=init_cfg)
...
class TargetAwareFusion(nn.Module):
    def __init__(self,
        ...
        # self.conv_cos_layers = []
        self.fusion_layers = []
        # self.downsample_layers = []
        self.mask_layers = []
        # self.taf2BU_transition_layers = []
        ...
        for idx, in_c in enumerate(in_channels):
            ...
            #                 nn.Sequential(*[ConvModule(in_c, 1, 1, norm_cfg=norm_cfg, act_cfg=act_cfg, bias=False),
            #                                 ConvModule(1, 1, 1, norm_cfg=norm_cfg, bias=False, act_cfg=dict(type='Sigmoid'))]))
            ...
            # self.add_module(f'convCosLayer{layer_idx}',
            #                 DepthwiseSeparableConvModule(in_c, in_c, 7, padding=7//2, bias=False,
            #                                              norm_cfg=norm_cfg, act_cfg=act_cfg))
            # self.conv_cos_layers.append(f'convCosLayer{layer_idx}')
            ...
            #                                                               planes=in_channels[layer_idx]//zxBottleneck.expansion,
            #                                                               stride=2,
            #                                                               norm_cfg=norm_cfg))
            #     self.downsample_layers.append(f'bottomUpLayer{layer_idx}')

            # from second fusion layer on, add a new conv module for bottom-up layer
            # if layer_idx>1:
            #     self.add_module(f'taf2BUTransitionLayer{layer_idx}', zxTAF2BUTransitionBlock(in_c,
            #                                                                                  planes=in_c//zxTAF2BUTransitionBlock.expansion,
            #                                                                                  stride=1,
            #                                                                                  norm_cfg=norm_cfg))
            #     self.taf2BU_transition_layers.append(f'taf2BUTransitionLayer{layer_idx}')
    ...
    def forward(self, rgb_x, lwir_x, gt_masks):
        ...
        for layer_idx, (tmp_rx, tmp_lx, tmp_fusion_name, tmp_mask_name, tmp_spaAtt_name) in \
            ...
            # else:
            #     tmp_taf2bu_layer = getattr(self, self.taf2BU_transition_layers[layer_idx-1])
            #     tmp_fused_res = tmp_taf2bu_layer(tmp_fused_res)
            #     bottomupLayer = getattr(self, f'bottomUpLayer{layer_idx}')
            #     bu_results.append(bottomupLayer(bu_results[-1], tmp_fused_res))
            ...
            if gt_masks is not None:
                loss_mask += self._neg_entropy(similarity_logits) * 1.0
