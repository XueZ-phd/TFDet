...
class FasterRCNN_RGBTwMask_wSpaAttV2_replaceFFMwithCatConv(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 dice_weight,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV2_replaceFFMwithCatConv, self).__init__(init_cfg=init_cfg)
...
from ..utils.csp_layer import DarknetBottleneck
class replaceCSP(BaseModule):
    def __init__(self,
                 in_channels,
                 out_channels,
                 conv_cfg=None,
                 ...
                 init_cfg=None):
        super(replaceCSP, self).__init__(init_cfg)
...
class TargetAwareFusion(nn.Module):
    def __init__(self,
        ...
        for idx, in_c in enumerate(in_channels):
            layer_idx = idx+1
            # fusion layers
            self.add_module(f'fusionLayer{layer_idx}', replaceCSP(in_c*2,
                                                                in_c,
                                                                norm_cfg=norm_cfg,
                                                                act_cfg=act_cfg))
    ...
    def forward(self, rgb_x, lwir_x, gt_masks):
        ...
        for layer_idx, (tmp_rx, tmp_lx, tmp_fusion_name, tmp_mask_name, tmp_spaAtt_name) in \
            ...
            # # noFRM_noLseg_noLnegentropy
            # tmp_spaAtt_layer = getattr(self, tmp_spaAtt_name)
            # bu_results_wSpaAtt.append(tmp_spaAtt_layer(tmp_fused_res))
