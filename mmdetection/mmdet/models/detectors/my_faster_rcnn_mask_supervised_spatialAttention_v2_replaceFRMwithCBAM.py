...
from ..utils import zxCBAM

@DETECTORS.register_module()
class FasterRCNN_RGBTwMask_wSpaAttV2_replaceFRMwithCBAM(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 dice_weight,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV2_replaceFRMwithCBAM, self).__init__(init_cfg=init_cfg)
...
class TargetAwareFusion(nn.Module):
    def __init__(self,
        ...
        for idx, in_c in enumerate(in_channels):
            ...
            # mask layers
            self.add_module(f'cbamLayer{layer_idx}', zxCBAM(in_c))
            self.cbamLayers.append(f'cbamLayer{layer_idx}')
    ...
    def forward(self, rgb_x, lwir_x, gt_masks):
        ...
        if gt_masks is not None:
            ...
            loss_mask = torch.tensor(0.0).to(lwir_x[0].device)

        bu_results_wSpaAtt = []
        assert len(rgb_x) == len(lwir_x) == len(self.fusion_layers) == len(self.cbamLayers)

        for layer_idx, (tmp_rx, tmp_lx, tmp_fusion_name, tmp_cbam_name) in \
                enumerate(zip(rgb_x, lwir_x, self.fusion_layers, self.cbamLayers)):

            tmp_fusion_layer = getattr(self, tmp_fusion_name)
            tmp_fused_res = tmp_fusion_layer(torch.cat([tmp_rx, tmp_lx], 1))

            # mask supervision
            tmp_cbam_layer = getattr(self, tmp_cbam_name)
            refined_tmp_fused_res, spatial_scale, channel_scale = tmp_cbam_layer(tmp_fused_res)
            bu_results_wSpaAtt.append(refined_tmp_fused_res)
            '''
            refined_tmp_fused_res是CBAM输出的结果。CBAM先做spatial attention，然后做channel attention。
            spatial_scale是spatial attention时的scale，是经过sigmoid的输出, size of [bs, 1, h, w]
            channel_gate是channel attention的scale，是没有经过sigmoid之前的张量, size of [bs, c, 1, 1]
            '''
            #
            ...
            if gt_masks is not None:
                ...
                gt_mask_1level = torch.from_numpy(resized_gt_mask).to(spatial_scale.device)
                assert gt_mask_1level.requires_grad is False, 'the ground-truth mask should not be updated'
                loss_mask_ = self.diceBCELoss(spatial_scale, gt_mask_1level)
                loss_mask += loss_mask_

            if gt_masks is not None:
                loss_mask += self._neg_entropy(channel_scale) * 1.0

        if gt_masks is not None:
            return loss_mask/len(self.cbamLayers), bu_results_wSpaAtt
