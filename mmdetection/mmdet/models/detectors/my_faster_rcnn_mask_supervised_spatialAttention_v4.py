...
class FasterRCNN_RGBTwMask_wSpaAttV4(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 taf_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV4, self).__init__(init_cfg=init_cfg)
...
class TargetAwareFusion(nn.Module):
    def __init__(self,
        ...
        for idx, in_c in enumerate(in_channels):
            # fusion layers
            self.add_module(f'csp_layer_{idx}', CSPLayer(in_c*2,
                                                         in_c,
                                                         expand_ratio=0.5,
                                                         norm_cfg=norm_cfg,
                                                         act_cfg=act_cfg))
            ...
            # self.add_module(f'spatial_layer_early_{idx}', SpatialGate(norm_cfg))
            self.add_module(f'spatial_layer_{idx}', SpatialGate(norm_cfg))
    ...
    def forward(self, rgb_x, lwir_x, gt_masks):
        ...
        for idx in range(len(rgb_x)):
            ...
            tmp_lx = lwir_x[idx]
            # csp fusion
            tmp_fused_res = getattr(self, f'csp_layer_{idx}')(torch.cat([tmp_rx, tmp_lx], 1))
            # tmp_fused_res = getattr(self, f'spatial_layer_early_{idx}')(tmp_fused_res)
            # mask supervision
