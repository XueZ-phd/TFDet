...
class FasterRCNN_RGBTwMask_wSpaAttV3(BaseDetector):
    def __init__(self,
                 share_weights,
                 backbone,
                 taf_cfg,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_RGBTwMask_wSpaAttV3, self).__init__(init_cfg=init_cfg)
...
class TargetAwareFusion(nn.Module):
    ...
    '''Ablation1: RGB和LWIR的被VGG16抽取的特征，经过CSP融合后直接输出到检测头'''
    # def forward(self, rgb_x, lwir_x, gt_masks):
    #     rgb_x = rgb_x if isinstance(rgb_x, tuple) else [rgb_x]
    #     lwir_x = lwir_x if isinstance(lwir_x, tuple) else [lwir_x]
    #
    ...
    #
    #     for layer_idx, (tmp_rx, tmp_lx, tmp_fusion_name) in \
    #             enumerate(zip(rgb_x, lwir_x, self.fusion_layers)):
    #
    #         tmp_fusion_layer = getattr(self, tmp_fusion_name)
    #         tmp_fused_res = tmp_fusion_layer(torch.cat([tmp_rx, tmp_lx], 1))
    #         bu_results_wSpaAtt.append(tmp_fused_res)
    #
    #     if gt_masks is not None:
    #         return torch.tensor(0.0).to(tmp_rx.device), bu_results_wSpaAtt
    #     return bu_results_wSpaAtt
