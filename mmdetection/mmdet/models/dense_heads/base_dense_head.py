...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self,
        ...
        """
        # outs[0]是FPN不同尺度特征图上的 scores. 单个特征图的score shape: [bs, 3*1, h, w]
        # outs[1]是FPN不同尺度特征图上的 bboxes. 单个特征图的score shape: [bs, 3*4, h, w]
        outs = self(x)  # RPN的前向过程
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
        # loss内部完成：
        # 1. anchor_generator: 产生anchors;
        # 2. assigner: 将gt分配给anchors，iou<0.3为负样本，0.3<=iou<0.7忽略，0.7<=iou正样本。类别索引从0开始。背景类别索引为num_cls的值
        # 3. sampler: 从上述所有正负样本中采样一部分固定数量的正负样本，规定正负样本的比例。这些值在train_cfg自定义
        # 4. loss_cls+loss_reg计算分类和回归的损失函数。
        # loss_cls是二分类交叉熵，所以分类时会将标签转换二分类标签。即前景二分类标签为1，背景二分类标签为0
        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore)
        if proposal_cfg is None:
            return losses
        else:
            # get_bboxes 可以传入score_factors,
            # When score_factors is not None, the cls_scores are usually multiplied by it
            # then obtain the real score used in NMS, such as CenterNess in FCOS, IoU branch in ATSS.
