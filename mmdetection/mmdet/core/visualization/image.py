...
def imshow_RGBT_det_bboxes(rgb_img,
                           lwir_img,
                           bboxes=None,
                           labels=None,
                           segms=None,
                           ...
                           out_file=None,
                           fig=None):
    """Draw bboxes and class labels (with scores) on a pair of RGBT image.
    ...
    rgb_img = mmcv.imread(rgb_img).astype(np.uint8)
    lwir_img = mmcv.imread(lwir_img).astype(np.uint8)
    ...
    lwir_img = mmcv.bgr2rgb(lwir_img)
    assert rgb_img.shape == lwir_img.shape
    # width, height = lwir_img.shape[1], lwir_img.shape[0]
    lwir_img = np.ascontiguousarray(lwir_img)
    rgb_img = np.ascontiguousarray(rgb_img)
    img = np.hstack([rgb_img, lwir_img])
    width, height = img.shape[1], img.shape[0]
    ...
    if bboxes is not None:
        num_bboxes = bboxes.shape[0]
        _bboxes = bboxes.copy()
        _bboxes[:, 0::2] = _bboxes[:, 0::2]+width//2
        bbox_palette = palette_val(get_palette(bbox_color, max_label + 1))
        colors = [bbox_palette[label] for label in labels[:num_bboxes]]
        draw_bboxes(ax, bboxes, colors, alpha=0.8, thickness=thickness)
        draw_bboxes(ax, _bboxes, colors, alpha=0.8, thickness=thickness)

        horizontal_alignment = 'left'
        positions = bboxes[:, :2].astype(np.int32) + thickness
        _positions = _bboxes[:, :2].astype(np.int32) + thickness
        areas = (bboxes[:, 3] - bboxes[:, 1]) * (bboxes[:, 2] - bboxes[:, 0])
        scales = _get_adaptive_scales(areas)
        scores = bboxes[:, 4] if bboxes.shape[1] == 5 else None
        for position in [positions, _positions]:
            draw_labels(
                ax,
                labels[:num_bboxes],
                position,
    ...
    if segms is not None:
        ...
        colors = [mask_palette[label] for label in labels]
        colors = np.array(colors, dtype=np.uint8)
        draw_masks(ax, img[:, :width//2, :], segms, colors, with_edge=True)
