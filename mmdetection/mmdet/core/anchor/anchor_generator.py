...
class AnchorGenerator:
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        Return:
            ...
            valid flags 表示的是采样哪些位置上的Anchors,
            具体说，FPN输出的特征图尺寸是网络决定的，一般由stride控制。
            AnchorHead产生的Anchor面积由config中定义的strides、scale决定，
            如果config中定义的strides与网络的strides相同，那么FPN每个位置产生的anchor都是valid flag，
            如果config中定义的strides与网络的strides不同，则由FPN的尺度和自定义的尺度的交集决定valid flag
        """
        assert self.num_levels == len(featmap_sizes)
        multi_level_flags = []
        for i in range(self.num_levels):
            ...
            h, w = pad_shape[:2]    # 图片padding之后的尺寸
