...
base_lr = 1e-2
bs = 4    # bs=3x4与bs=10差不多,但是bs=3x4速度是bs=10的3倍左右.
...
depth = 34
model = dict(
    type='FasterRCNN_RGBTwMask_shareConv_notShareBN',
    backbone=dict(
        # type='ResNet',
        type='ResNet_ShareConv_notShareBN',
        depth=depth,
        init_cfg=dict(type='Pretrained', checkpoint=f'torchvision://resnet{depth}'),
        base_channels=64,
        num_stages=4,
        strides=(1, 2, 2, 1),
        dilations=(1, 1, 1, 2),
