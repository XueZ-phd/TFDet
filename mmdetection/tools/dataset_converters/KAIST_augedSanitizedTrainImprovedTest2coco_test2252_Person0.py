...
parser.add_argument('--train_image_root', type=str,
                    default='/home/zx/cross-modality-det/datasets/KAIST-zxAugedTrainOriTest/train_images',
                    help='the root path to training image path',)
parser.add_argument('--test_image_root', type=str,
                    default='/home/zx/cross-modality-det/datasets/KAIST-zxAugedTrainOriTest/test_images',
                    help='the root path to training image path',)
parser.add_argument('--annotation_root', type=str,
                    default='/home/zx/cross-modality-det/datasets/KAIST-zxAugedTrainOriTest/annos',
                    help='the root path to train (sanitized) and test (improved) annotations',)
parser.add_argument('--out_path', type=str,
                    default='/home/zx/cross-modality-det/datasets/KAIST-zxAugedTrainOriTest/augedTrain_coco_format',
...
def get_images_generator(mode):
    if mode == 'train':
        # train
        train_annotation_names = sorted(glob(osp.join(args.annotation_root, 'train_annos', '*.txt')))
        assert len(train_annotation_names) == 14888
        train_image_names = []
        for train_anno_name in train_annotation_names:
            train_anno_name = osp.basename(train_anno_name)
            set_id, v_id, I_id = train_anno_name.split('_', 2)
        ...
        test_annotation_names = sorted(glob(osp.join(args.annotation_root, 'annotations_KAIST_test_set', '*.txt')))
        ...
        for test_anno_name in test_annotation_names:
            ...
            set_id, v_id, I_id = test_anno_name.split('_', 2)
...
def parse_txt(image_path, img_w, img_h):
    ...
    anno_path = osp.join(args.annotation_root, sep, anno_path)
