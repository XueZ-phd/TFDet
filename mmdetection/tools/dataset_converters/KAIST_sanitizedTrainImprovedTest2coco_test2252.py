...
def parse_txt(image_path):
    ...
    with warnings.catch_warnings():
        ...
        _all_bboxes = np.loadtxt(anno_path, skiprows=1, usecols=[1, 2, 3, 4], ndmin=2)
        _all_labels = np.loadtxt(anno_path, skiprows=1, usecols=0, dtype=str, ndmin=1)
        # remove annotations whose labels are not in classes
        valid_idx = [_ in list(cls2id_dict.keys()) for _ in _all_labels]
        _all_bboxes = _all_bboxes[valid_idx]
        _all_labels = _all_labels[valid_idx]
        if _all_bboxes.shape[0]>0:
            # valid bboxes have size greater than 0, top-left point is inside the valid region
            areas = _all_bboxes[:, 2] * _all_bboxes[:, 3]
            _all_bboxes = _all_bboxes[areas > 0]
            _all_bboxes[:, 0] = np.maximum(_all_bboxes[:, 0], 0)
            _all_bboxes[:, 1] = np.maximum(_all_bboxes[:, 1], 0)
            all_labels = _all_labels[areas > 0]

    if _all_bboxes.shape[0] != 0:
        all_bboxes = copy.deepcopy(_all_bboxes)
        bbox_heights = _all_bboxes[:, 3]
        bbox_aspects = _all_bboxes[:, 3]/_all_bboxes[:, 2]
        all_bboxes[:, 2:4] = _all_bboxes[:, 0:2] + _all_bboxes[:, 2:4]
        assert np.array(_ in list(cls2id_dict.keys()) for _ in all_labels).all()

        condition = []
        # ignore losses generated by categories 'people', 'person?', 'person?a'
        assert len(all_bboxes) == len(all_labels)
        for _lbl, _bbox, box_h, box_a in zip(all_labels, all_bboxes, bbox_heights, bbox_aspects):
            # if cls2id_dict[_lbl] == cls2id_dict['person'] and box_h >= 50:
            if cls2id_dict[_lbl] == cls2id_dict['person'] and box_h >= 0:
                condition.append(True)
            else:
                condition.append(False)

        labels = [lbl for cond, lbl in zip(condition, all_labels) if cond]
        bboxes = [box for cond, box in zip(condition, all_bboxes) if cond]

        labels_ignore = [lbl for cond, lbl in zip(condition, all_labels) if not cond]
        bboxes_ignore = [box for cond, box in zip(condition, all_bboxes) if not cond]
    else:
        bboxes = []
        bboxes_ignore = []
