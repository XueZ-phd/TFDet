...
from mmdet.core.visualization import imshow_RGBT_det_bboxes, imshow_det_bboxes
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['DefaultFormatBundleRGBT', 'NormalizeRGBT', 'CollectRGBT', 'RandomFlipRGBT'],
...
def main():
    ...
    for _idx, item in enumerate(dataset):
        ...
        if len(gt_bboxes):
            assert (gt_labels == 0).all()
            assert (np.min(gt_bboxes[:, -1]-gt_bboxes[:, 1]) > 0.)
        ...
        if cfg.load_RGBT:
            fig = plt.figure('RGBT', frameon=False)
            imshow_RGBT_det_bboxes(
                item['rgb_img'],
                item['lwir_img'],
                gt_bboxes,
                gt_labels,
