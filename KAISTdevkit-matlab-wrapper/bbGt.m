...
% zx
% thres=0.01
% dt = dt(dt(:, 6)==1 | dt(:, 5)<=thres, :); % 仅保存TPs和Score<=thres的框，即删除Score>thres的FPs
% compute results
...
if( roc )
  ...
  for i=1:m, j=find(xs1<=ref(i)); ref(i)=ys1(j(end)); end   % 先选择最靠近参考点的FPPI的索引j(end)，然后取对应的recall(j(end))
else
